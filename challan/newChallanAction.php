...
include($_SERVER['DOCUMENT_ROOT']."/$omenNX/var.php");
  ...
  if($action=="fetchcustomerdetail"){
    ...
    $sqlquery="INSERT INTO challan_tbl (
						DATE,customer_id,BILL_ID)
						VALUES 
						('".$billdate."',".$customer_id.",null)";                                                           
	
	$show=mysqli_query($dbhandle,$sqlquery);
	
	
	if($show==1){
		$show=mysqli_query($dbhandle,"select * from challan_tbl ORDER BY challan_no DESC;");
		$row=mysqli_fetch_array($show);
		$challan_no=$row["challan_no"];
		$billItemList=explode("--",$quantityList);
		foreach($billItemList as $billItem){
						if($billItem!=""){
						$sqlquery="INSERT INTO challan_items_tbl
						(challan_no,quantity,RATE,DESCRIPTION)
						VALUES
						($challan_no,$billItem,140,'mix lot')";
						
						$show=mysqli_query($dbhandle,$sqlquery);
						}
							
	}
			$sqlquery="INSERT INTO challan_transport_tbl
			(challan_no,transport_name,transport_parcels)
			VALUES
			($challan_no,'$transportname',$transportparcels)";
			$show=mysqli_query($dbhandle,$sqlquery);
